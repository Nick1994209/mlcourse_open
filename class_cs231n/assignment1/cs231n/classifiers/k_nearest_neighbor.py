...
class KNearestNeighbor(object):
    ...
    def compute_distances_two_loops(self, X: np.array):
        ...
        for i in xrange(num_test):
            for j in xrange(num_train):
                ...
                #####################################################################
                # dists[i, j] = np.sqrt(np.sum((dists[i] - self.X_train[j]) ** 2))
                # print(np.sqrt(np.sum((dists[i] - self.X_train[j]) ** 2)))
    ...
    def predict_labels(self, dists, k=1):
        ...
        for i in xrange(num_test):
            ...
            #########################################################################
            indexes_sorted_values = np.argsort(dists[i])[:k]
            closest_y = self.y_train[indexes_sorted_values]
            #########################################################################
            # TODO:                                                                 #
